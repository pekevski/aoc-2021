...
last_input = nil;
complete_boards = []
incomplete_boards = bingo_boards.clone()

bingo_input.each() do |input|

        # Mark each input across the boards
        incomplete_boards.each{|board| 
            board.mark(input)
            board.debug()
            puts " => Unmarked Sum: #{board.sum_unmmarked()}, Bingo: #{board.has_bingo()}" 
        }

        # After we have marked, check for bingo
        # For every board that has bingo, remove it from the list.
        incomplete_boards.each(){|board| 
        ...
        # remove any boards that have bingo
        incomplete_boards = incomplete_boards.filter {|b| !b.has_bingo()}
        
        if incomplete_boards.empty?
            last_input = input
            break
        end
end

last_board = complete_boards.last()

puts "Last board #{last_board.get_id()} has BINGO! Answer is #{last_board.sum_unmmarked()} * #{last_input} = #{last_board.sum_unmmarked() * last_input}"
